...
| **R-03** | `FileSandbox` does not intercept package saves, so atomic multi-asset rollback does not work as ADR-0005 designs. | Medium | Low | WS-11 | **Partial close:** q1/q3 POSITIVE at runtime for Content/ adds + full `Discard` (`Rollback.MultiAssetDiscard` green, `0e2e78d`). Residual: BP compile/CDO, deletions, `Saved/`/`Config/`, `DiscardFiles` hazard. Scoped `rollback.available: true` only. | open |
