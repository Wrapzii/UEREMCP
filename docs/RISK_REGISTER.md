...
| **R-06** | **Duplicating Epic.** 27 engine toolsets already cover many named domains; workstreams rebuild working tools. | Medium | Medium | WS-02 | **Partial close:** `source_complete` audits + WS-09 GAS/tags/DataTable dispositions folded (`ae03dc5`). Runtime MCP schema dumps still open. Keep rule 2; do not treat schemas as runtime-verified. | open |
