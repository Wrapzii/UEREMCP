...
The first primitive baseline was attempted live using
...
trial. The fixture fix landed in `866248a`; WS-14's clean measured rerun is in
flight.
...
- an acceptance rule for `partially_completed` versus validated completion;
...
current-lineage evidence bundle was subsequently refreshed on `d033dd8` with
B1–B10 **PASS**. The executable, clean measured primitive baseline remains open.
**No overall POC-B claim.**
...
The refreshed `d033dd8` bundle now records B1 transport **PASS** and B2–B10
**PASS**. Overall POC-B remains unclaimed pending WS-14's clean measured
primitive-baseline trial, unavailable token accounting, and resolution of any
acceptance rule governing `partially_completed` versus validated completion.
