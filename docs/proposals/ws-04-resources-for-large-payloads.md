...
complement for bulk `complete` payloads via `IModelContextProtocolResourceProvider`,
keeping the envelope `result` authoritative for status/validation/metrics.

Not normative for every `complete` response yet — domains may inline until measured
payload sizes force the link path (R-09). Implementation owner: graph-serializing
domain (WS-06 for Blueprint first; WS-07/WS-08 for their graphs) with transport
registration help from WS-04. No ADR-0004 schema change required for Wave 1.
