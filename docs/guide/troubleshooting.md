...
| `partially_completed` on Niagara create | Domain skipped a check, timed out, or queued | Read `capability_notes` / `checks_skipped`; B10 warm-pixel is a separate gate — do not invent visual PASS from this status |
| `partially_completed` + `job` | Timeout path (ADR-0009) | Poll `get_job_result`; cancel via `cancel_job` if `cancellable: true` |
...
| `execute_plan` missing as tool | Module not loaded / Live Coding | Rebuild/enable UEREMCP; `list_toolsets` for `UeremcpReference` / `ExecutePlan` |
