...
def check_benchmark(contract: dict, errors: list[str]) -> int:
    ...
    for case in contract.get("routing_benchmark", []):
        ...
        if intent_id == "execute_plan_misroute":
            ...
            if not any(r.get("id") == "execute_plan" for r in routes):
                fail(errors, "benchmark execute_plan_misroute: special_routes.execute_plan missing")
            if routes and any(
                r.get("id") == "execute_plan" and r.get("agent_facing_aicallable") is not False
                for r in routes
            ):
                fail(errors, "execute_plan must remain agent_facing_aicallable=false")
