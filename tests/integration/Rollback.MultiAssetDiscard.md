...
| **Shipping** UEREMCP plugin gate (`UEREMCP.Validation.*`, UEREMCP enabled) | **Green** `[VERIFIED-RUNTIME: tests/integration/_logs/editor_UEREMCP_Validation_shipping.redacted.md]` — `Rollback.MultiAssetDiscard` and `Harness.Smoke` `Result={Success}` (re-run 2026-07-30; exit 0). |
